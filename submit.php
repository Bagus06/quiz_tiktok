...
function imageUploadFromFile(array $f,string $label,array &$errors):?array{
    ...
    return['tmp'=>$tmp,'mime'=>$mime,'width'=>$width,'height'=>$height];
}
...
if(!$files||!is_array($files['name']??null)||count($files['name'])!==3){$errors[]='Jawaban nomor 10 wajib berisi tepat 3 gambar.';}else{for($i=0;$i<3;$i++){$file=['name'=>$files['name'][$i]??'','type'=>$files['type'][$i]??'','tmp_name'=>$files['tmp_name'][$i]??'','error'=>$files['error'][$i]??UPLOAD_ERR_NO_FILE,'size'=>$files['size'][$i]??0];$img=imageUploadFromFile($file,'Gambar jawaban nomor 10 ke-'.($i+1),$errors);if($img)$q10Images[]=$img;}}
if(!$errors){$c=db()->prepare('SELECT id FROM participants WHERE LOWER(name)=LOWER(?) OR whatsapp=? LIMIT 1');$c->execute([$name,$wa]);if($c->fetch())$errors[]='Nama atau nomor WhatsApp sudah pernah digunakan.';}
$log=db()->prepare('INSERT INTO submission_attempts(ip_address,whatsapp,was_successful) VALUES(?,?,0)');$log->execute([$ip,$wa?:null]);$attemptId=(int)db()->lastInsertId();
...
$pdo=db();$pdo->beginTransaction();$s=$pdo->prepare('INSERT INTO participants(name,name_normalized,whatsapp,tiktok_account,tiktok_profile_url,subscriber_photo,comment_photo,token,submit_ip,submitted_at) VALUES(?,?,?,?,?,?,?,?,?,NOW())');$token='';$inserted=false;for($tokenAttempt=0;$tokenAttempt<25;$tokenAttempt++){$token=participantTokenCandidate();try{$s->execute([$name,$nameNormalized,$wa,$tiktokAccount,$url,$sp,$cp,$token,$ip]);$inserted=true;break;}catch(PDOException $insertError){if(!isDuplicateKeyError($insertError)||stripos($insertError->getMessage(),'token')===false)throw $insertError;}}if(!$inserted)throw new RuntimeException('Gagal membuat token unik.');
$pid=(int)$pdo->lastInsertId();$a=$pdo->prepare('INSERT INTO participant_answers(participant_id,question_id,answer_text) VALUES(?,?,?)');$imgIns=$pdo->prepare('INSERT INTO participant_answer_images(participant_answer_id,image_path,sort_order) VALUES(?,?,?)');foreach($qrows as $q){$qid=(int)$q['id'];$number=(int)$q['question_number'];$text=$number===10?'[3 gambar diunggah]':trim((string)$answers[$qid]);$a->execute([$pid,$qid,$text]);$answerId=(int)$pdo->lastInsertId();if($number===10){foreach($q10Paths as $i=>$path)$imgIns->execute([$answerId,$path,$i+1]);}}
$pdo->prepare('UPDATE submission_attempts SET was_successful=1 WHERE id=?')->execute([$attemptId]);$pdo->commit();$_SESSION['last_submit_time']=time();unset($_SESSION['old']);$_SESSION['success_token']=$token;$_SESSION['success_name']=$name;header('Location:success.php');exit;
}catch(Throwable $e){if(isset($pdo)&&$pdo instanceof PDO&&$pdo->inTransaction())$pdo->rollBack();foreach($stored as $file)@unlink($file);$_SESSION['errors']=['Data gagal disimpan. Pastikan database telah diperbarui dan folder upload dapat ditulis.'];header('Location:index.php');exit;}
