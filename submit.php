...
if(!$errors){$proofHashes=array_merge([(string)$sub['hash'],(string)$com['hash']],array_column($q10Images,'hash'));$placeholders=implode(',',array_fill(0,count($proofHashes),'?'));$duplicateProof=db()->prepare("SELECT 1 FROM participants WHERE subscriber_image_hash IN ($placeholders) OR comment_image_hash IN ($placeholders) LIMIT 1");$duplicateProof->execute(array_merge($proofHashes,$proofHashes));$duplicateAnswer=db()->prepare("SELECT 1 FROM participant_answer_images WHERE image_hash IN ($placeholders) LIMIT 1");$duplicateAnswer->execute($proofHashes);if($duplicateProof->fetchColumn()||$duplicateAnswer->fetchColumn())$errors[]='Salah satu gambar sudah pernah digunakan oleh peserta lain.';}
...
}catch(Throwable $e){if(isset($pdo)&&$pdo instanceof PDO&&$pdo->inTransaction())$pdo->rollBack();foreach($stored as $file)@unlink($file);$_SESSION['errors']=$e instanceof UnexpectedValueException&&$e->getMessage()==='DAILY_QUOTA_FULL'?['Mohon maaf, kuota peserta untuk hari ini baru saja terpenuhi. Silakan mencoba kembali besok.']:['Data gagal disimpan. Pastikan database telah diperbarui dan folder upload dapat ditulis.'];header('Location:index.php');exit;}
